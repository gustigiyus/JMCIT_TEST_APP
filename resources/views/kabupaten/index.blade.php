...
@section('content')
    <div class="max-w-3xl mx-auto flex flex-col gap-4">
        ...
        <h1 class="w-full  text-center font-black text-3xl">Data Kabupaten</h1>
    ...
    <script>
        $(document).ready(function() {
            ...
            function clearValidator() {
                ...
                $('.nama_kabupaten_msg').removeClass(
                    'text-red-600 dark:text-red-500 text-green-600 dark:text-green-500');
                $('.nama_kabupaten_msg').html('');

                $('.provinsi_id_label').removeClass(
                    'text-red-700 dark:text-red-500 text-green-700 dark:text-green-500');
                $('.provinsi_id').removeClass(
                    'bg-red-50 border border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500 focus:border-red-500 dark:text-red-500 dark:placeholder-red-500 dark:border-red-500 is-invalid bg-green-50 border border-green-500 text-green-900 placeholder-green-700 focus:ring-green-500 focus:border-green-500 dark:text-green-500 dark:placeholder-green-500 dark:border-green-500'
                );
                $('.provinsi_id_msg').removeClass(
                    'text-red-600 dark:text-red-500 text-green-600 dark:text-green-500');
                $('.provinsi_id_msg').html('');
            }
            ...
            $('body').on('click', '.tombol-update', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#nama_kabupaten').val(response.data.nama)
            ...
            function save(action, id) {
                ...
                let url_data = '';
                let nama_kabupaten = $('.nama_kabupaten').val()
                let nama_provinsi = $('.provinsi_id').val()
                console.log(nama_provinsi)

                // Menambahkan data ke dalam objek FormData
                formData.append('nama', nama_kabupaten);
                formData.append('provinsi_id', nama_provinsi);

                if (action == 'simpan') {
                    url_data = 'kabupaten'
                } else if (action == 'update') {
                    url_data = 'kabupaten/' + id
                } else {
                    return false
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (res) {
                            clearValidator()
                            $('#nama_kabupaten').addClass('is-valid');
                            $('.nama_kabupaten_msg').addClass('valid-feedback');
                    ...
                    error: function(response) {
                        ...
                        if (res) {
                            ...
                            Swal.fire({
                                ...
                                if (res.nama) {
                                    $('.nama_kabupaten_label').addClass(
                                        'text-red-700 dark:text-red-500'
                                    )
                                    $('.nama_kabupaten').addClass(
                                    ...
                                    $('.nama_kabupaten_msg').addClass(
                                        'text-red-600 dark:text-red-500')
                                    $('.nama_kabupaten_msg').html(res.nama)
                                } else {
                                    $('.nama_kabupaten_label').removeClass(
                                ...
                                if (res.provinsi_id) {
                                    $('.provinsi_id_label').addClass(
                                        'text-red-700 dark:text-red-500'
                                    )
                                    $('.provinsi_id').addClass(
                                        'bg-red-50 border border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500 focus:border-red-500 dark:text-red-500 dark:placeholder-red-500 dark:border-red-500'
                                    )
                                    $('.provinsi_id_msg').addClass(
                                        'text-red-600 dark:text-red-500')
                                    $('.provinsi_id_msg').html(res.provinsi_id)
                                } else {
                                    $('.provinsi_id_label').removeClass(
                                        'text-red-700 dark:text-red-500').addClass(
                                        'text-green-700 dark:text-green-500'
