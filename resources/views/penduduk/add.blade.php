...
@section('content')
    ...
    <div class="flex flex-row mx-auto justify-center !mt-20">
        <div class="max-w-3xl flex flex-col gap-4 bg-slate-300 shadow w-full items-center p-0">
            <h1 class="w-full text-center font-black text-3xl bg-slate-400 py-3">Form Penduduk</h1>
            ...
            <div class="px-6 py-6 w-full ">
                <form action="{{ route('penduduk_store') }}" method="POST" class="flex flex-col gap-5">
                    ...
                    <div class="flex flex-row gap-5">
                        ...
                        <input type="hidden" id="default_value_provinsi_id_error"
                            value="{{ Session::get('provinsi_id') }}">

                        <input type="hidden" id="default_value_kabupaten_id_error"
                            value="{{ Session::get('kabupaten_id') }}">
                    </div>
                    ...
                    <div class="flex flex-row gap-5">
                        ...
                        <input type="hidden" id="default_value_jns_kelamin_error"
                            value="{{ Session::get('jns_kelamin') }}">
                    </div>
                    ...
                    <div class="w-full">
                        ...
                        <textarea id="alamat" rows="4" name="alamat"
                            ...
                            placeholder="Masukan data alamat...">{{ Session::get('alamat') }}</textarea>
    ...
    <script>
        $(document).ready(function() {
            ...
            if (deafult_provinsi_id) {

                if (deafult_provinsi_id) {
                    $('#provinsi_id').val(deafult_provinsi_id)
                    $.ajax({
                        type: "get",
                        url: "getKabupaten/" + deafult_provinsi_id,
                        headers: {
                        ...
                        success: function(response) {
                            ...
                            $.each(dataKabupaten, function(index, kabupaten) {
                                $('#kabupaten_id').append(new Option(kabupaten.nama,
                                    kabupaten.id));
                            });
                ...
                setTimeout(function() {
                    if (deafult_kabupaten_id) {
                        $('#kabupaten_id').val(deafult_kabupaten_id)
                    } else {
                        $('#kabupaten_id').val('')
                    }
                }, 300);
